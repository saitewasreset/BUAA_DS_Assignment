...
struct Graph *Graph_fromPattern(size_t vertexCount, size_t edgeCount) {
    ...
    for (size_t i = 0; i < edgeCount; i++) {
        ...
        Graph_insertEdge(graph, id, vertexA, vertexB, d);
    }
...
void Graph_insertEdge(struct Graph *graph, size_t id, size_t vertexA,
                      size_t vertexB, int weight) {
    ...
    for (size_t i = 0; i < graph->vertexCapacity; i++) {
        ...
        while (current != NULL) {
            if (!visited[i][current->otherVertex]) {
                sprintf(nodeFormatBuffer,
                        "%zu -- %zu [label = \"%zu, %d\";];\n", i,
                        current->otherVertex, current->id, current->weight);
                strcat(outputBuffer, nodeFormatBuffer);
...
struct Graph *Graph_MSTPrim(struct Graph *graph, size_t beginVertex,
                            size_t *ptrToMinWeight, size_t *edgeIdList) {
    size_t N = graph->vertexCapacity;
    size_t minWeight = 0;
    size_t edgeIdListLen = 0;
    ...
    while (1) {
        ...
        size_t edgeId = 0;
        struct GraphEdgeNode *current = graph->vertexList[minIndex].edgeList;
        while (current != NULL) {
            if (current->otherVertex == edges[minIndex]) {
                edgeId = current->id;
            }
            current = current->next;
        }

        Graph_insertEdge(mstGraph, edgeId, lastId, currentId, minW);
        minWeight += (size_t)minW;
        edgeIdList[edgeIdListLen] = edgeId;
        edgeIdListLen++;
