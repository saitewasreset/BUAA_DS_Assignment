...
struct BinaryTree *BinaryTree_fromRPN(const char *rpn) {
    ...
    for (size_t i = 0; i < rpnLen; i++) {
        if (isdigit(rpn[i])) {
            ...
            struct BinaryTree_Node *node = (struct BinaryTree_Node *)malloc(
                sizeof(struct BinaryTree_Node));
            nodeCount++;
            node->data = (BinaryTree_Data *)malloc(sizeof(BinaryTree_Data));
            *(node->data) = num;
            ...
            stackTop--;
            struct BinaryTree_Node *t1 = nodeStack[stackTop];
            stackTop--;
            struct BinaryTree_Node *t2 = nodeStack[stackTop];
